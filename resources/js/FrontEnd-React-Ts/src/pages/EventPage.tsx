...
import { PayAccount, getPaymentAccounts, generateOrderCode, submitOrder, trackOrder } from "@/lib/api/order";
              ...
              {trackedOrder && (
                ...
                >
                  ...
                  <CardContent className="p-6 space-y-4 pt-8">
                    <div>
                      <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{isEn ? 'Order Code' : 'Kode Order'}</p>
                      <p className="font-mono font-extrabold text-2xl text-primary mt-1">{trackedOrder.order_code}</p>
                    </div>
                    <div>
                      <p className="text-sm font-semibold leading-tight line-clamp-2">{trackedOrder.event?.title_id || 'Unknown Event'}</p>
                    </div>
                    <div className="flex justify-between items-center">
                      <Badge variant="outline" className={`rounded-md border-0 ${
                        trackedOrder.status === 'success' ? 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400' :
                        trackedOrder.status === 'pending' ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400' : 'bg-rose-500/15 text-rose-600 dark:text-rose-400'
                      }`}>
                    ...
                    <div className="pt-3 border-t border-dashed">
                      <p className="text-lg font-bold text-right text-foreground">{formatPrice(trackedOrder.total_price)}</p>
                    </div>
                    <div className="bg-primary/5 text-primary text-center py-2 rounded-lg mt-2 group-hover:bg-primary/10 transition-colors">
                      <p className="text-xs font-semibold uppercase">{isEn ? 'Click to view ticket' : 'Ketuk untuk lihat e-tiket'}</p>
                    </div>
                  </CardContent>
                </Card>
              )}
      ...
      <section>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="w-[95vw] max-w-[500px] max-h-[90vh] overflow-y-auto rounded-xl [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            <DialogHeader>
              <DialogTitle>
                {isEn ? "Ticket Order" : "Pesan Tiket"}: {isEn ? (selectedEvent?.title_en || selectedEvent?.title_id) : selectedEvent?.title_id}
              </DialogTitle>
              {!isSuccess && (
                <DialogDescription>
                  {isEn ? "Please fill the form below to place your order." : "Silakan isi form di bawah ini untuk memesan tiket."}
                </DialogDescription>
              )}
            </DialogHeader>
            ...
            {isSuccess ? (
              <div className="flex flex-col items-center justify-center py-6 text-center space-y-4">
                ...
                <div className="space-y-1">
                  <h3 className="font-semibold text-xl">{isEn ? "Order Submitted!" : "Pesanan Berhasil Dibuat!"}</h3>
                  <p className="text-muted-foreground text-sm">
                    {isEn ? "Your order code is" : "Kode Pesanan Anda adalah"} <span className="font-bold text-foreground">{orderCode}</span>.
                  </p>
                  <p className="text-muted-foreground text-sm">
                    {isEn ? "Please keep this code to check your order status." : "Simpan kode ini untuk memantau status pesanan Anda."}
                  </p>
                </div>
                <Button onClick={() => setIsDialogOpen(false)} className="mt-4 bg-gradient-gold text-primary-foreground">
                  {isEn ? "Close" : "Tutup"}
                </Button>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-6 py-2">
                ...
                <div className="bg-muted/50 p-4 rounded-lg space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs text-muted-foreground">{isEn ? "Your Order Code" : "Kode Order Anda"}</p>
                      <p className="font-mono font-bold text-lg tracking-wider text-primary">{orderCode || '...'}</p>
                    </div>
                    <Button type="button" variant="ghost" size="sm" onClick={copyOrderCode} disabled={!orderCode}>
                ...
                <div className="grid gap-4">
                  <div className="grid gap-2">
                    <Label htmlFor="name">{isEn ? "Full Name" : "Nama Lengkap"} <span className="text-red-500">*</span></Label>
                    <Input id="name" required value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder={isEn ? "Your full name" : "Nama lengkap anda"} />
                  </div>
                  
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="grid gap-2">
                      <Label htmlFor="email">Email <span className="text-red-500">*</span></Label>
                      <Input id="email" type="email" required value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} placeholder="example@mail.com" />
                    </div>
                    <div className="grid gap-2">
                      <Label htmlFor="phone">WhatsApp <span className="text-red-500">*</span></Label>
                      <Input id="phone" type="tel" required value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} placeholder="081234567890" />
                    </div>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="grid gap-2">
                      <Label htmlFor="qty">{isEn ? "Ticket Qty" : "Jumlah Tiket"} <span className="text-red-500">*</span></Label>
                  ...
                  {(selectedEvent?.price && selectedEvent.price > 0) ? (
                    <>
                      <div className="grid gap-2">
                        <Label>{isEn ? "Payment Method" : "Metode Pembayaran"} <span className="text-red-500">*</span></Label>
                        <Select required value={formData.payment_method} onValueChange={(val) => setFormData({...formData, payment_method: val})}>
                          <SelectTrigger>
                            <SelectValue placeholder={isEn ? "Select transfer destination" : "Pilih tujuan transfer"} />
                          </SelectTrigger>
                          <SelectContent>
                            {accounts.map(acc => (
                              <SelectItem key={acc.id} value={`${acc.name_account} - ${acc.no_account} - ${acc.batch_member?.name || 'Unknown'}`}>
                                {acc.name_account} - {acc.no_account} - {acc.batch_member?.name || 'Unknown'}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      ...
                      <div className="grid gap-2">
                        <Label htmlFor="proof">{isEn ? "Payment Proof" : "Bukti Pembayaran"} <span className="text-red-500">*</span></Label>
                        <Input 
                          id="proof" 
                          type="file" 
                          accept="image/*" 
                  ...
                  <div className="grid gap-2">
                    <Label htmlFor="notes">{isEn ? "Additional Notes" : "Catatan (Opsional)"}</Label>
                    <Textarea 
                      id="notes" 
                      rows={2} 
            ...
            {trackedOrder && (
              <div className="flex flex-col relative">
                ...
                <div className="bg-gradient-gold p-8 text-primary-foreground relative overflow-hidden flex flex-col items-center justify-center text-center">
                  ...
                  <h2 className="text-3xl font-black mb-2 font-display tracking-widest">{isEn ? 'E-TICKET' : 'E-TIKET'}</h2>
                  <Badge variant="secondary" className="bg-white/25 hover:bg-white/30 border-none text-white shadow-none uppercase tracking-widest font-bold px-3 py-1">
                    {trackedOrder.status}
                  </Badge>
                </div>
                ...
                <div className="px-8 py-8 bg-card relative">
                  ...
                  <div className="space-y-6">
                    <div>
                      <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">{isEn ? 'Event' : 'Acara'}</p>
                      <p className="font-bold text-lg leading-tight">{trackedOrder.event?.title_id || 'Unknown Event'}</p>
                    </div>
                    
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">{isEn ? 'Name' : 'Nama'}</p>
                        <p className="font-semibold text-sm truncate">{trackedOrder.name}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">{isEn ? 'Ticket Qty' : 'Jml Tiket'}</p>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">{isEn ? 'Location' : 'Lokasi'}</p>
                        <p className="font-semibold text-sm truncate">{trackedOrder.event?.location_id || '-'}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">{isEn ? 'Date' : 'Tanggal'}</p>
                        <p className="font-semibold text-sm">
                          {trackedOrder.event?.date ? new Date(trackedOrder.event.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }) : '-'}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-6 pt-5 border-t border-dashed flex flex-col items-center justify-center">
                    ...
                    <p className="font-mono text-lg tracking-[0.2em] font-extrabold text-center text-primary">{trackedOrder.order_code}</p>
                  </div>
                </div>
