...
import { Table as EventTable } from "./Feature/Event/Components/Table";
import { FormModal as EventFormModal } from "./Feature/Event/Components/FormModal";
import { DetailSheet as EventDetailSheet } from "./Feature/Event/Components/DetailSheet";
import { DeleteDialog as EventDeleteDialog } from "./Feature/Event/Components/DeleteDialog";
  ...
  return (
    <DashboardLayout>
      ...
      <div className="flex flex-col gap-4 max-w-7xl mx-auto pb-12 relative">
        ...
        {/* Controls Row (Hidden when on Event tab since EventTable has its own TanStack toolbar) */}
        {activeTab !== "event" && (
          <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-3">
            ...
            <div className="relative w-full lg:w-80">
              ...
              <Input
                placeholder={
                  activeTab === "order"
                    ? "Cari kode order atau pemesan..."
                    : "Cari bank atau nama pemilik..."
                }
            ...
            <div className="flex flex-row flex-wrap sm:flex-nowrap items-center gap-2 w-full lg:w-auto">
              ...
              {hasRole(["Developer", "Admin"]) && (
                <Button
                  ...
                  onClick={
                    activeTab === "order" ? handleAddOfflineOrder : handleAddAccount
                  }
                >
                  <Plus className="h-3.5 w-3.5 mr-1" />
                  {activeTab === "order" ? "Pesan Tiket" : "Tambah Rekening"}
                </Button>
              )}
            </div>
          </div>
                  ...
                  <TableRow>
                    ...
                    <TableHead>
                      <Button
                        variant="ghost"
                        className="-ml-4 hover:bg-transparent"
                        onClick={() => handleSort("event.title_id")}
                      >
                              ...
                              <>
                                {ord.order_method !== "offline" && (
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() => handleEditOrderStatus(ord)}
                  ...
                  <TableRow>
                    ...
                    <TableHead>
                      <Button
                        variant="ghost"
                        className="-ml-4 hover:bg-transparent"
                        onClick={() => handleSort("name_account")}
                      >
                    ...
                    <TableHead>
                      <Button
                        variant="ghost"
                        className="-ml-4 hover:bg-transparent"
                        onClick={() => handleSort("batch_member.name")}
                      >
                  ...
                  {filteredAccounts.length === 0 ? (
                    ...
                    filteredAccounts.map((acc) => (
                      <TableRow key={acc.id} className="transition-colors">
                        <TableCell>
                          <span
                            className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-semibold capitalize ${acc.type === "bank" ? "bg-blue-500/10 text-blue-700" : "bg-emerald-500/10 text-emerald-700"}`}
                          >
                            {acc.type}
                          </span>
                        </TableCell>
        ...
        <EventFormModal
          ...
          eventErrors={eventErrors as any}
            ...
            <form onSubmit={handleSubmitOrderStatus} className="space-y-4 pt-2">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(val: any) => setOrderData("status", val)}
                >
            ...
            <form onSubmit={handleSubmitAccount} className="space-y-4 pt-2">
              <div>
                ...
                {accountErrors.batch_member_id && (
                  <span className="text-xs text-red-500">{accountErrors.batch_member_id}</span>
                )}
              </div>
        ...
        <Sheet open={isViewSheetOpen} onOpenChange={setIsViewSheetOpen}>
          <SheetContent className="overflow-y-auto [&::-webkit-scrollbar]:hidden">
            <SheetHeader className="mb-4">
              ...
              <SheetDescription>
                {viewingEvent
                  ? "Informasi lengkap acara & pendaftaran."
                  : "Verifikasi rincian transaksi & bukti transfer."}
              </SheetDescription>
            </SheetHeader>

        <EventDetailSheet
          isOpen={isViewSheetOpen && viewingEvent !== null}
          event={viewingEvent}
          onClose={() => {
