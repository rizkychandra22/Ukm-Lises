...
export default function IndexGallery({ galleries }: PageProps) {
    ...
    const [sortConfig, setSortConfig] = useState<{ key: 'title_id' | 'desc_id' | 'status', direction: 'asc' | 'desc' } | null>(null);
        ...
        <DashboardLayout>
            ...
            <div className="flex flex-col gap-4 max-w-7xl mx-auto pb-12 relative">
                ...
                <div className="flex justify-between items-start">
                    <div>
                        <h2 className="text-2xl font-bold text-foreground font-display tracking-tight">Manajemen Data Galeri</h2>
                ...
                <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-3">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 w-full lg:w-auto">
                        <Button onClick={handleAdd} size="sm" className="h-8 px-3.5 rounded-lg text-[13px] font-medium w-full sm:w-auto shrink-0 shadow-sm">
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead>
                                    <Button variant="ghost" className="-ml-4 hover:bg-transparent" onClick={() => handleSort('desc_id')}>
                                        Deskripsi <ArrowUpDown className="ml-2 h-4 w-4" />
                                    </Button>
                                </TableHead>
                                <TableHead>
                                    <Button variant="ghost" className="-ml-4 hover:bg-transparent" onClick={() => handleSort('status')}>
                        ...
                        <TableBody>
                            {filteredAndSortedGalleries.length === 0 ? (
                                <TableRow>
                                    <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">Tidak ada galeri yang ditemukan.</TableCell>
                                </TableRow>
                            ) : (
                                    ...
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell className="text-right">
                                            <div className="flex justify-end gap-2 items-center">
                                                <Button className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg h-8 w-8 p-0" size="sm" onClick={() => handleEdit(item)} title="Edit Galeri">
                                                    <Edit className="h-4 w-4" />
                                                </Button>
                                                <Button variant="destructive" size="sm" className="rounded-lg h-8 w-8 p-0" onClick={() => handleDelete(item)} title="Hapus Galeri">
                ...
                {galleries.last_page > 1 && (
                    <div className="mt-4">
                        <Pagination>
                            <PaginationContent>
                                <PaginationItem>
                                    <PaginationPrevious href={galleries.prev_page_url || "#"} className={!galleries.prev_page_url ? "pointer-events-none opacity-50" : ""} />
                                </PaginationItem>
                                {galleries.links.filter(l => !l.label.includes('&laquo;') && !l.label.includes('&raquo;')).map((link, i) => (
                                ...
                                <PaginationItem>
                                    <PaginationNext href={galleries.next_page_url || "#"} className={!galleries.next_page_url ? "pointer-events-none opacity-50" : ""} />
                                </PaginationItem>
                            </PaginationContent>
                        </Pagination>
                    </div>
            ...
            <Dialog open={isSheetOpen} onOpenChange={setIsSheetOpen}>
                <DialogContent className="w-[90%] max-w-[500px] rounded-md">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4 pt-4 max-h-[75vh] overflow-y-auto no-scrollbar px-1">
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea value={formData.desc_id} onChange={e => setFormData({ ...formData, desc_id: e.target.value })} className="h-24" />
                                {errors.desc_id && <span className="text-xs text-destructive">{errors.desc_id}</span>}
                            </div>
                            <div className="space-y-2">
                                <Label>Gambar Galeri {selectedGallery && <span className="bg-background text-sm file:text-foreground file:bg-muted file:border-0 file:rounded-md file:px-2 file:py-1 file:mr-2">(Kosongkan jika tidak diubah)</span>}</Label>
                                <Input type="file" 
                            ...
                            <div className="space-y-4 pt-4 border-t">
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="space-y-0.5">
                                        ...
                                        <p className="text-xs text-muted-foreground">Tampil paling awal & besar di Halaman Galeri.</p>
                                    </div>
                                    <Switch checked={formData.is_index} onCheckedChange={(checked) => setFormData({ ...formData, is_index: checked })} />
                                </div>
                            </div>
                        </div>
