            ...
            {releases && releases.length > 0 ? (
              <Accordion type="single" collapsible className="w-full grid grid-cols-1 lg:grid-cols-2 gap-4">
                {releases.map((release) => (
                  <AccordionItem key={release.id} value={`item-${release.id}`} className="border rounded-md px-4 bg-background shadow-sm">
                    <AccordionTrigger className="hover:no-underline hover:bg-muted/50 rounded-t-md transition-colors -mx-4 px-4">
                      <div className="flex flex-row items-center text-left gap-2 sm:gap-3">
                        <Badge variant="default" className="w-fit mr-1">{release.version}</Badge>
                    ...
                    <AccordionContent className="pt-2 pb-4 text-muted-foreground border-t mt-1">
                      {release.subtitle && <p className="font-semibold text-foreground mb-2">{release.subtitle}</p>}
