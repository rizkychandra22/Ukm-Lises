...
export default function IndexMember({ members, batches, majors }: Props) {
  ...
  const [sortBatchConfig, setSortBatchConfig] = useState<{
    key: "year" | "name_id";
    direction: "asc" | "desc";
  } | null>(null);
  ...
  const [sortMemberConfig, setSortMemberConfig] = useState<{
    key: "name" | "major_id" | "periode" | "position_id" | "batch_year" | "batch_name";
    direction: "asc" | "desc";
  } | null>(null);
  ...
  const filteredBatches = useMemo(() => {
    ...
    if (searchBatch) {
      ...
      result = result.filter(
        (b) =>
          b.year.toLowerCase().includes(lowercased) || b.name_id.toLowerCase().includes(lowercased),
      );
    }
        ...
        <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-3">
          ...
          <div className="relative w-full lg:w-80">
            ...
            <Input
              placeholder={
                activeTab === "anggota"
                  ? "Cari anggota berdasarkan nama atau prodi..."
                  : "Cari angkatan..."
              }
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 w-full lg:w-auto">
            ...
            {(() => {
              ...
              <Button
                ...
                onClick={activeTab === "anggota" ? handleAddMember : handleAddBatch}
              >
                <Plus className="h-3.5 w-3.5" /> Tambah{" "}
                {activeTab === "anggota" ? "Anggota" : "Angkatan"}
              </Button>
            )}
          </div>
        </div>
              ...
              <Table>
                  ...
                  <TableRow>
                    ...
                    <TableHead>
                      <Button
                        variant="ghost"
                        className="-ml-4 hover:bg-transparent"
                        onClick={() => handleSortMember("major_id")}
                      >
                        Jurusan
                        <ArrowUpDown className="ml-2 h-4 w-4" />
                      </Button>
                      ...
                      <>
                        <TableHead>
                          <Button
                            variant="ghost"
                            className="-ml-4 hover:bg-transparent"
                            onClick={() => handleSortMember("periode")}
                          >
                            Periode
                            <ArrowUpDown className="ml-2 h-4 w-4" />
                          </Button>
                        ...
                        <TableHead>
                          <Button
                            variant="ghost"
                            className="-ml-4 hover:bg-transparent"
                            onClick={() => handleSortMember("position_id")}
                          >
                            Jabatan
                            <ArrowUpDown className="ml-2 h-4 w-4" />
                          </Button>
                    ...
                    <TableHead>
                      <Button
                        variant="ghost"
                        className="-ml-4 hover:bg-transparent"
                        onClick={() => handleSortMember("batch_year")}
                      >
                        Tahun
                        <ArrowUpDown className="ml-2 h-4 w-4" />
                      </Button>
                    ...
                    <TableHead>
                      <Button
                        variant="ghost"
                        className="-ml-4 hover:bg-transparent"
                        onClick={() => handleSortMember("batch_name")}
                      >
                        Nama Angkatan
                        <ArrowUpDown className="ml-2 h-4 w-4" />
                      </Button>
                ...
                <TableBody>
                  {filteredMembers.length === 0 ? (
                    <TableRow>
                      <TableCell
                        colSpan={showPeriodeJabatan ? 8 : 6}
                  ...
                  <TableRow>
                    ...
                    <TableHead>
                      <Button
                        variant="ghost"
                        className="-ml-4 hover:bg-transparent"
                        onClick={() => handleSortBatch("name_id")}
                      >
                        Nama Angkatan
                        <ArrowUpDown className="ml-2 h-4 w-4" />
                      </Button>
                  ...
                  {filteredBatches.length === 0 ? (
                    ...
                    filteredBatches.map((b) => (
                      <TableRow key={b.id}>
                        ...
                        <TableCell>
                          <span
                            className={`text-xs px-2 py-1 rounded-full font-medium ${b.status === "Deactive" ? "bg-amber-500/10 text-amber-600 border border-amber-500/20" : "bg-emerald-500/10 text-emerald-600 border border-emerald-500/20"}`}
                          >
              ...
              {!editingBatch && (
                <div className="flex items-start gap-2 bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20 p-3 rounded-md">
                  ...
                  <p className="text-xs leading-relaxed">
                    Pemberitahuan: Sistem akan otomatis membuatkan akun akses untuk angkatan ini
                    dengan Username{" "}
                    <span className="font-semibold">lises{batchData.year || "202X"}</span> dan
              ...
              {editingBatch && (
                <>
                  <div className="flex items-start gap-2 bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20 p-3 rounded-md">
                    <Info className="h-4 w-4 mt-0.5 shrink-0" />
                    <p className="text-xs leading-relaxed">
                      Opsional: Isi kolom di bawah jika ingin mengubah akses akun untuk angkatan
                      ini. (Biarkan kosong jika tidak diubah)
                    </p>
                  </div>
                  <div>
          ...
          <AlertDialogContent className="w-[90%] max-w-[360px] rounded-md p-6">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="text-center text-[14px] mt-2 mb-4 text-foreground/80">
                Apakah Anda yakin? Tindakan ini akan menghapus data angkatan beserta seluruh anggota
                di dalamnya.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <div className="flex flex-row justify-center gap-3 mt-2">
              ...
              <AlertDialogAction
                ...
                className="w-24 h-8 text-[13px] font-medium rounded-lg bg-destructive text-destructive-foreground hover:bg-destructive/90"
              >
                Hapus
              </AlertDialogAction>
            </div>
          ...
          <DialogContent className="w-[90%] max-w-[500px] rounded-md">
            ...
            >
              <div>
                ...
                />
                {memberErrors.name && (
                  <span className="text-xs text-red-500">{memberErrors.name}</span>
                )}
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  {memberErrors.major_id && (
                    <span className="text-xs text-red-500 mt-1 block">{memberErrors.major_id}</span>
                  )}
                </div>
              </div>
                  ...
                  <Select
                    ...
                    onValueChange={(val) => {
                      ...
                      setMemberData((data) => {
                        ...
                        const isBatchValid = newBatches.some(
                          (b) => b.id.toString() === data.batch_id,
                        );
              ...
              <div>
                ...
                {memberErrors.batch_id && (
                  <span className="text-xs text-red-500">{memberErrors.batch_id}</span>
                )}
              </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          required
                        />
                        {memberErrors.periode && (
                          <span className="text-xs text-red-500">{memberErrors.periode}</span>
                        )}
                      </div>
                    </div>
                    ...
                    {orgStatusValue === "Kepengurusan" && (
                      <div>
                        ...
                        <Input
                          ...
                          required
                        />
                        {memberErrors.position_id && (
                          <span className="text-xs text-red-500">{memberErrors.position_id}</span>
                        )}
                      </div>
                    )}
                  </>
              ...
              <div>
                ...
                <Input
                  ...
                  accept="image/*"
                  onChange={(e) =>
                    setMemberData("image", e.target.files ? e.target.files[0] : null)
                  }
                />
                {memberErrors.image && (
                  <span className="text-xs text-red-500">{memberErrors.image}</span>
                )}
              </div>
                ...
                <div className="flex justify-center">
                  <img
                    src={
                      viewingMember.image
                        ? viewingMember.image
                        : `https://ui-avatars.com/api/?name=${encodeURIComponent(viewingMember.name)}&background=random`
                    }
                ...
                <div className="grid grid-cols-2 gap-y-3 gap-x-4">
                  ...
                  <div className="col-span-2">
                    ...
                    <p className="text-base font-medium">
                      {viewingMember.major
                        ? `${viewingMember.major.degree ? viewingMember.major.degree + " - " : ""}${viewingMember.major.name_id}`
                        : viewingMember.major_id}
                    </p>
                  </div>
                  <div className="col-span-2">
                    <h4 className="text-sm font-medium text-muted-foreground">
                      Angkatan Tahun <b>{viewingMember.batch?.year}</b>
                    </h4>
                  ...
                  <div className={viewingMember.type === "Pengurus" ? "col-span-1" : "col-span-2"}>
                    ...
                    <p className="text-base font-medium flex items-center">
                      ...
                      <span
                        className={`ml-2 text-xs leading-none px-2 py-0.5 rounded-full ${viewingMember.status === "Active" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                      >
